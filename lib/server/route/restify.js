...
Restify.prototype._buildController = function (verb, Model, fields) {
  ...
  function _buildFindQuery(req, id) {
    var condition = { $or: [] };
    if (id.match(/^[0-9]{4}[0-9a-fA-F]{20}$/)) {
      condition.$or.push({ _id: id });
    } else if (id.match(/^[a-z][-a-z0-9]*$/)) { // slug (objectId always starts a 4-byte value representing the seconds since the Unix epoch)
      condition.$or.push({ slug: id });
    } else {
...
Restify.prototype.model = function (path, modelName, overrideController, auth) {
  var self = this,
